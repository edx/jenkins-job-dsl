...
class AnalyticsConstants {
    ...
    public static def common_parameters = { extraVars, env=[:] ->
        return {
            stringParam('CLUSTER_NAME', env.get('CLUSTER_NAME', extraVars.get('CLUSTER_NAME')), 'Name of the EMR cluster to use for this job.')
...
This text may reference other parameters in the task as shell variables, e.g.  $$CLUSTER_NAME./$)
            ...
            stringParam('NUM_TASK_CAPACITY', env.get('NUM_TASK_CAPACITY', extraVars.get('NUM_TASK_CAPACITY')), 'Number of EMR instance capacity to use for this job.')
