...
dbt seed --full-refresh --profiles-dir $WORKSPACE/profiles/ --profile $DBT_PROFILE --target $DBT_TARGET

if [ "$WITH_SNAPSHOT" == "true" ]
then
    dbt snapshot --profiles-dir $WORKSPACE/profiles/ --profile $DBT_PROFILE --target $DBT_TARGET
fi

dbt run $DBT_RUN_OPTIONS $DBT_RUN_EXCLUDE --profiles-dir $WORKSPACE/profiles/ --profile $DBT_PROFILE --target $DBT_TARGET
...
dbt test $DBT_TEST_OPTIONS $DBT_TEST_EXCLUDE --profiles-dir $WORKSPACE/profiles/ --profile $DBT_PROFILE --target $DBT_TARGET && true
if [ $? -eq 1 ]
then
    if [ "$WITH_RETRY" == "true" ]
    then
        ...
        if [ "$DBT_TEST_EXCLUDE" == "" ]
        then
            python ../../tools/ci_scripts/rerun_flaky_tests.py --project-path . --profiles-dir $WORKSPACE/profiles/  --profile $DBT_PROFILE  \
