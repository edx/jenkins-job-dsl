    ...
    public static def job = { dslFactory, allVars ->
        ...
        dslFactory.job("warehouse-transforms-worker-ci"){
            ...
            parameters {
                ...
                stringParam('SECOND_LAST_MERGE_COMMIT', allVars.get('SECOND_LAST_MERGE_COMMIT'), 'Provide Second last merge commit id of Warehouse Transform repo.')
            }
            ...
            publishers {
                downstreamParameterized {
                    trigger("warehouse-transforms-ci-master-merges-rerun") {
                        condition('FAILED')
                        parameters {
        ...
        dslFactory.job("warehouse-transforms-ci-master-merges-rerun"){
            authorization common_authorization(allVars)
            logRotator common_log_rotator(allVars)
            parameters secure_scm_parameters(allVars)
            parameters {
