        ...
        allVars.get('SNOWFLAKE_READ_REPLICA_IMPORTS').each { db, db_config ->
            dslFactory.job("snowflake-${db.toLowerCase()}-read-replica-import") {
                ...
                parameters {
                    ...
                    stringParam('SCRATCH_SCHEMA', db_config.get('SCRATCH_SCHEMA', allVars.get('SCRATCH_SCHEMA')), 'ScratchSchema')
                    stringParam('RUN_ID', db_config.get('RUN_ID', allVars.get('RUN_ID')))
